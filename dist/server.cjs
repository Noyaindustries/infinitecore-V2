...
var STAFF_CHAT_NOTIFY_EMAIL_THROTTLE_MS = 3 * 60 * 1e3;
...
Code parrainage : ${referralCode}`
  ...
  app2.post("/api/chats/staff-notify", async (req, res) => {
    try {
      ...
      const preview = messagePreviewRaw.length > 200 ? `${messagePreviewRaw.slice(0, 200)}...` : messagePreviewRaw;
      ...
      const shouldEmail = !Number.isFinite(lastStaffMailAt) || lastStaffMailAt <= 0 || nowMs - lastStaffMailAt >= STAFF_CHAT_NOTIFY_EMAIL_THROTTLE_MS;
      if (shouldEmail) {
        void sendStaffNotifyEmail({
  ...
  app2.post("/api/auth/admin-role", async (req, res) => {
    try {
      ...
      if (!VALID_ROLES.has(requestedRoleRaw)) {
        return res.status(400).json({
          success: false,
          error: `R\xF4le invalide. R\xF4les autoris\xE9s : ${[...VALID_ROLES].join(", ")}.`
        });
      }
      ...
      if (target.uid === auth.uid && requestedRoleRaw !== "admin") {
        return res.status(400).json({
          success: false,
          error: "Vous ne pouvez pas modifier votre propre r\xF4le d'admin."
        });
      }
      if (target.role === "admin" && requestedRoleRaw !== "admin") {
        ...
        if (adminsCount <= 1) {
          return res.status(400).json({
            success: false,
            error: "Impossible de r\xE9trograder le dernier administrateur."
          });
        }
      }
...
function subscriptionStatusFromStripe(raw) {
  ...
  if (["active", "trialing", "past_due", "incomplete", "incomplete_expired", "unpaid"].includes(status)) {
    if (status === "unpaid") return "Impay\xE9";
    return "Actif";
  }
...
async function createExpressApplication() {
  ...
  const resolveStripeCustomerId = async (auth) => {
  ...
  app2.post("/api/stripe/webhook", async (req, res) => {
    try {
      ...
      const upsertOrderPatch = async (orderId, patch) => {
        const existing = await prisma.dataDocument.findUnique({
          where: {
            collectionPath_docId: { collectionPath: ORDERS_COLLECTION_PATH, docId: orderId }
          }
      ...
      const updateBySubscriptionId = async (subscriptionId, patch) => {
        const rows = await prisma.dataDocument.findMany({
      ...
      switch (event.type) {
        ...
        case "invoice.payment_failed": {
          ...
          const subscriptionId = typeof rawInvoice.subscription === "string" ? rawInvoice.subscription : String(rawInvoice.subscription?.id || "");
          if (subscriptionId) {
            await updateBySubscriptionId(subscriptionId, {
              status: "Impay\xE9",
  ...
  const persistPaddeAuditToStores = async (data, options = {}) => {
    ...
    const clientName = String(
      firstFilled("clientname", "client_name", "nom", "name", "entreprise", "company", "company_name") ?? "Client PADDE-CI"
    ).trim();
    ...
    if (!appEnv.node.isProduction) {
      console.info("[padde-ci] payload re\xE7u", {
  ...
  app2.post("/api/webhooks/padde-ci/direct", async (req, res) => {
    try {
      ...
      return res.status(200).json({ success: true, message: "Audit PADDE-CI enregistr\xE9.", auditId });
    } catch (error) {
